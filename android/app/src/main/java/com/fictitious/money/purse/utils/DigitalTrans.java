...
//        System.out.println("===16进制cardOutPut==="+DigitalTrans.byte2hex(App.cardOutPut));
    ...
    public static byte[] decKeyByAES128CBC(byte[] ciphertext, String key, int coinType) {
        ...
        if (CommonUtil.isMessyCode(decResultStr)) {
            int length = 64;
            if (coinType == Constants.COIN_TYPE.TYPE_EOS || coinType == Constants.COIN_TYPE.TYPE_GPS) {
                length = 51;
            } else if (coinType == Constants.COIN_TYPE.TYPE_ETH || coinType == Constants.COIN_TYPE.TYPE_VNS) {
                length = 64;
            } else if (coinType == Constants.COIN_TYPE.TYPE_BTC || coinType == Constants.COIN_TYPE.TYPE_LTC || coinType == Constants.COIN_TYPE.TYPE_USDT) {
                length = 52;
            } else if (coinType == Constants.COIN_TYPE.TYPE_BTM) {
                length = 128;
            }
